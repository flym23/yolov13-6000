#!/usr/bin/env bash
# L3-CRU T1--T7: reuse original-YOLOv13 references and run three deterministic seed workers on device 0.
set -Eeuo pipefail
...
PREFLIGHT_MARKER="$TRAIN_DIR/.l3cru_preflight_ready.marker"
LEGACY_PREFLIGHT_MARKER="$ROOT/runs/l3cru/l3cru_preflight_ready.marker"
LOCK="$STATE_DIR/.${RUN_ID}.chain_lock"
# The two original-YOLOv13 summaries below are reused as the baseline; never retrain T0.
ABLATION_GROUPS=(t1_amsc t2_bgdr t3_ugdr t4_amsc_bgdr t5_amsc_ugdr t6_bgdr_ugdr t7_full)
...
if [[ ! -f "$PREFLIGHT_MARKER" && -f "$LEGACY_PREFLIGHT_MARKER" ]]; then
  cp "$LEGACY_PREFLIGHT_MARKER" "$PREFLIGHT_MARKER"
fi
...
for seed in 0 1 2; do
  "$PY" "$ROOT/tools/run_l3cru_ablation.py" --yaml-dir "$YAML_DIR" --data "$DATA" --device 0 \
    --groups "${ABLATION_GROUPS[@]}" --seeds "$seed" --epochs 300 --patience 40 --imgsz 640 --batch 16 --workers 2 \
    --project "$TRAIN_DIR" --run-id "$RUN_ID" --test-command "$TEST_COMMAND" \
    >"$TRAIN_LOG_DIR/$RUN_ID.seed$seed.log" 2>&1 &
  PIDS+=("$!")
done
printf '%s\n' "${PIDS[@]}" >"$STATE_DIR/$RUN_ID.worker.pids"
wait_group "${PIDS[@]}"
write_state complete "t1_t7_finished=true; original_yolov13_reused=true; three_parallel_seed_workers=true"
