...
CURRENT_STAGE=smoke_c5
SMOKE_NAME="${RUN_ID}_smoke_c5_seed0"
write_state smoke_training "one_epoch_c5_seed0"
"$PY" "$ROOT/tools/train_cmrf_worker.py" --root "$ROOT" --stage c5 --data "$DATA" --name "$SMOKE_NAME" \
  --seed 0 --epochs 1 --patience 1 >"$TRAIN_DIR/$SMOKE_NAME.log" 2>&1
SMOKE_WEIGHTS="$TRAIN_DIR/$SMOKE_NAME/weights/best.pt"
[[ -f "$SMOKE_WEIGHTS" ]] || { echo "CMRF smoke trial did not create $SMOKE_WEIGHTS" >&2; exit 77; }
write_state smoke_testing "one_epoch_c5_seed0"
"$PY" "$ROOT/test.py" --weights "$SMOKE_WEIGHTS" --data "$DATA" --name "$SMOKE_NAME" --device 0 --batch 16 --imgsz 640 --workers 2 \
  >"$TEST_DIR/$SMOKE_NAME.log" 2>&1
[[ -f "$TEST_DIR/$SMOKE_NAME/summary_metrics.json" ]] || { echo "CMRF smoke validation summary missing" >&2; exit 77; }
