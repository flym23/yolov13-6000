...
for stage in "${STAGES[@]}"; do
  ...
  for seed in 0 1 2; do
    ...
    CUDA_VISIBLE_DEVICES=0 WANDB_DISABLED=true "$PY" "$ROOT/tools/train_dcra_worker.py" \
      --root "$ROOT" --stage "$stage" --data "$DATA" --name "$name" --seed "$seed" --epochs 200 \
      > "$TRAIN/${name}.log" 2>&1 &
    PIDS+=("$!")
