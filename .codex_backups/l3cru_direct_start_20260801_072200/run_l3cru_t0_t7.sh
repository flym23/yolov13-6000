#!/usr/bin/env bash
# L3-CRU T1--T7: CARM-aligned pretrained, stage-synchronous three-seed protocol.
set -Eeuo pipefail
...
write_state() {
  "$PY" -c 'import json,sys; from datetime import datetime,timezone; from pathlib import Path; p=Path(sys.argv[1]); d={"run_id":sys.argv[2],"status":sys.argv[3],"stage":sys.argv[4],"detail":sys.argv[5],"launcher_pid":int(sys.argv[6]),"dependency":{"carm_state":sys.argv[7]},"references":{"l0":sys.argv[8],"p0":sys.argv[9]},"initialization":{"method":"YOLO.load","pretrained":sys.argv[10]},"completed_stages":sys.argv[11:],"settings":{"epochs":300,"patience":40,"device":0,"workers":2,"amp":False,"deterministic":True,"plots":False,"imgsz":640,"batch":16,"parallel_seed_processes":3,"stage_synchronous":True},"updated_at":datetime.now(timezone.utc).isoformat()}; p.write_text(json.dumps(d,ensure_ascii=False,indent=2),encoding="utf-8")' "$STATE_FILE" "$RUN_ID" "$1" "$CURRENT_STAGE" "${2:-}" "$$" "$CARM_STATE_FILE" "$L0_REFERENCE" "$P0_REFERENCE" "$PRETRAINED" "${COMPLETED[@]}"
}
...
[[ -x "$PY" && -f "$DATA" && -f "$PRETRAINED" && -f "$CARM_STATE_FILE" && -f "$L0_REFERENCE" && -f "$P0_REFERENCE" ]] || {
...
write_state running "carm_complete=true; original_yolov13_reused=true; initialization=yolov13n.pt"
...
for stage in "${STAGES[@]}"; do
  CURRENT_STAGE="$stage"
  write_state training "initialization=yolov13n.pt; three_parallel_seeds=true"
  TRAIN_PIDS=()
