...
if [[ "$SKIP_SMOKE" != "true" ]]; then
  prepare_dataset_cache
  CURRENT=smoke.a4_tau020.train
  write_state running "$CURRENT"
  SMOKE=dcra_${RUN_ID}_smoke_a4_tau020_seed0
  train_one a4_tau020 0 0 1 "$SMOKE"
  CURRENT=smoke.a4_tau020.test
  write_state running "$CURRENT"
  test_one "$SMOKE" 0
  audit_one "$SMOKE" 0
  ...
  write_state smoke_complete a4_tau020
fi
