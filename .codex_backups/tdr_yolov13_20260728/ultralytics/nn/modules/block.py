...
class TIERDCRAUp(DCRAUp):
    ...
    def _compute_reliability(self, weights, lateral):
        ...
        if self.finite_fallback:
            fallback = torch.nan_to_num(entropy_conf, nan=0.0, posinf=1.0, neginf=0.0)
            reliability = torch.where(torch.isfinite(reliability), reliability, fallback)
    ...
    def _compute_alignment(self, deep, lateral):
        ...
        residual = (reassembled - base) * reliability.to(device=deep.device, dtype=deep.dtype)
