#!/usr/bin/env bash
# After the MESA chain exits, run original YOLOv13 (L0) then LCER-DCRA L3 on UDD.
set -Eeuo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PY="${PYTHON_BIN:-/home/room305/.conda/envs/yolov13/bin/python}"
DATA="/home/room305/ZZF/UDD/data.yaml"
MESA_CHAIN_PATTERN='[r]un_mesa_ablation\.sh'
...
ALL_STAGES=(l0_baseline l3_local_consensus)
...
mkdir "$LOCK" 2>/dev/null || { echo "UDD LCER-DCRA chain already running" >&2; exit 73; }

write_state() {
  local status="$1"
  local stage="$2"
  local code="$3"
  LCER_DCRA_STATUS="$status" LCER_DCRA_STAGE="$stage" LCER_DCRA_CODE="$code" \
    LCER_DCRA_RUN_ID="$RUN_ID" LCER_DCRA_PID="$PID" LCER_DCRA_STATE="$STATE" \
    LCER_DCRA_DATA="$DATA" LCER_DCRA_STAGE_ORDER="$STAGE_ORDER_CSV" \
...
payload = {
    ...
    "completed_stages": [item for item in os.environ["LCER_DCRA_COMPLETED"].split(",") if item],
    ...
    "start_condition": "MESA chain process has exited",
...
run_stage() {
  ...
  COMPLETED+=("$stage")
  "$PY" "$ROOT/tools/collect_lcer_dcra_ablation.py" --root "$ROOT" --run-id "$RUN_ID" --data "$DATA" --stages "${COMPLETED[@]}"
}
...
CURRENT="waiting_mesa"
write_state waiting "$CURRENT" 0
while pgrep -f "$MESA_CHAIN_PATTERN" >/dev/null; do
  echo "$(date -u +%FT%TZ) waiting for MESA chain to finish"
  sleep 60
done
...
echo "UDD LCER-DCRA L0/L3 chain completed: run_id=$RUN_ID"
