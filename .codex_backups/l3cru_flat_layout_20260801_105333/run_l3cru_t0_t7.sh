...
STATE_DIR="$TRAIN_DIR/l3cru_chain"
TRAIN_LOG_DIR="$TRAIN_DIR"
RUN_ID=${L3CRU_RUN_ID:-l3cru_$(date -u +%Y%m%d_%H%M%S)}
STATE_FILE="$STATE_DIR/$RUN_ID.state.json"
# Keep the validated module preflight marker separate from experiment outputs.
PREFLIGHT_MARKER="$ROOT/runs/l3cru/l3cru_preflight_ready.marker"
...
for seed in 0 1 2; do
  "$PY" "$ROOT/tools/run_l3cru_ablation.py" --yaml-dir "$YAML_DIR" --data "$DATA" --device 0 \
    ...
    --project "$TRAIN_DIR" --run-id "${RUN_ID}_seed${seed}" --test-command "$TEST_COMMAND" \
