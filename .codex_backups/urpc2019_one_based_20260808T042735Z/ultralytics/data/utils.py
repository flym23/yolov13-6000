...
def verify_image_label(args):
    ...
    im_file, lb_file, prefix, keypoint, num_cls, nkpt, ndim, label_index_offset = args
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            if nl := len(lb):
                ...
                # Model targets are always 0-based. A dataset YAML with names keyed 1..nc
                # declares that its label txt files use those same 1-based ids.
                max_cls = lb[:, 0].max()  # max label id
                min_cls = lb[:, 0].min()
                if label_index_offset:
                    assert label_index_offset == 1, f"Unsupported label index offset: {label_index_offset}"
                    assert min_cls >= label_index_offset and max_cls <= num_cls - 1 + label_index_offset, (
                        f"Label class ids out of expected 1-based range [1, {num_cls}]. "
                        f"Got min={int(min_cls)}, max={int(max_cls)}."
                    )
                    lb[:, 0] -= label_index_offset
                else:
                    # Retain backward-compatible validation for ordinary 0-based datasets;
                    # cache_labels() will auto-fix an unambiguous legacy 1-based set.
...
def check_det_dataset(dataset, autodownload=True):
    ...
    # URPC2019 stores semantic names and YOLO txt labels with 1-based ids (1..nc).
    # Preserve that on-disk convention and record the offset here; YOLODataset applies
    # it while parsing each label before targets reach the model (which is 0-based).
    names = data["names"]
    if isinstance(names, dict):
        names = {int(k): str(v) for k, v in names.items()}
        nc = len(names)
        if set(names) == set(range(1, nc + 1)):
            data["names"] = {index - 1: name for index, name in names.items()}
            data["label_index_offset"] = 1
            LOGGER.info(
                f"Dataset YAML uses 1-based class ids 1-{nc}; labels will be converted to internal 0-{nc - 1}."
            )
...
def save_dataset_cache_file(prefix, path, x, version):
    """Save an Ultralytics dataset cache atomically so parallel trainers never share a temporary file."""
    x["version"] = version  # add cache version
    if is_dir_writeable(path.parent):
        # ``np.save(path, ...)`` appends ``.npy`` and therefore made every
        # process write the same ``*.cache.npy`` file.  Three concurrent
        # trainers could then move/remove each other's temporary cache.
        temporary = path.with_name(f".{path.name}.{os.getpid()}.tmp.npy")
        try:
            np.save(str(temporary), x)
            temporary.replace(path)  # atomic replacement on the shared filesystem
        finally:
