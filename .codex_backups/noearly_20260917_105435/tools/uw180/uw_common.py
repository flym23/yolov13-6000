...
SEEDS = (0, 1, 2)
TRAIN_ARGS = dict(epochs=180, device=0, workers=2, amp=False,
...
def required_files(variant):
    return [DATA, WEIGHTS, model_yaml(variant), Path(__file__).with_name('uw_worker.py'),
            Path(__file__).with_name('uw_common.py'), Path(__file__).with_name('uw_chain.py'),
