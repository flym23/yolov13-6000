...
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
  echo "AARG chain lock already exists: $LOCK_DIR" >&2
  exit 73
fi
...
write_state() {
  ...
  local stage="$2"
  local code="${3:-0}"
  AARG_STATUS="$status" AARG_STAGE="$stage" AARG_CODE="$code" AARG_RUN_ID="$RUN_ID" \
    AARG_LAUNCHER_PID="$$" AARG_STATE_DIR="$STATE_DIR" "$PY" - <<'PY'
import json
import os
from datetime import datetime, timezone
from pathlib import Path

path = Path(os.environ["AARG_STATE_DIR"]) / "state.json"
payload = {
    "run_id": os.environ["AARG_RUN_ID"],
    "status": os.environ["AARG_STATUS"],
    "stage": os.environ["AARG_STAGE"],
    "exit_code": int(os.environ["AARG_CODE"]),
    "launcher_pid": int(os.environ["AARG_LAUNCHER_PID"]),
    "updated_at": datetime.now(timezone.utc).isoformat(),
    "stage_order": ["e3_full", "e1_aarup", "e2_p2guide"],
}
...
run_train() {
  ...
  local gpu="$2"
  local seed="$3"
  local name="aarg_${RUN_ID}_${stage}_seed${seed}"
  local log="$TRAIN_DIR/${name}.log"

  CUDA_VISIBLE_DEVICES="$gpu" WANDB_DISABLED=true "$PY" "$ROOT/tools/train_aarg_worker.py" \
...
run_group() {
  ...
  "$PY" "$ROOT/tools/collect_aarg_ablation.py" --root "$ROOT" --run-id "$RUN_ID" --stages "$stage"
  write_state "stage_complete" "$stage"
}

STAGES=()
for stage in e3_full e1_aarup e2_p2guide; do
  run_group "$stage"
  STAGES+=("$stage")
done
...
write_state "running" "$CURRENT_STAGE"
"$PY" "$ROOT/tools/collect_aarg_ablation.py" --root "$ROOT" --run-id "$RUN_ID" --stages "${STAGES[@]}"

CURRENT_STAGE="complete"
write_state "complete" "$CURRENT_STAGE"
echo "AARG ablation chain completed: run_id=$RUN_ID stages=${STAGES[*]}"
