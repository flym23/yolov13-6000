#!/usr/bin/env python3
"""Collect SCAF validation summaries into JSON and CSV files."""
...
STAGE_CONFIGS = {
    "s4_scaf": {
        "yaml": "yolov13_scaf.yaml",
        "structure": "FAAR-B3 + SCAFFuse(P5->P4 semantic, consistency) + SCAFFuse(P4->P3 detail, consistency), Detect(P3,P4,P5)",
    },
    "s5_scaf_no_consistency": {
        "yaml": "yolov13_scaf_no_consistency.yaml",
        "structure": "FAAR-B3 + SCAFFuse(P5->P4 semantic, no consistency) + SCAFFuse(P4->P3 detail, no consistency), Detect(P3,P4,P5)",
    },
    "s2_scaf_p4": {
        "yaml": "yolov13_scaf_p4.yaml",
        "structure": "FAAR-B3 + SCAFFuse(P5->P4 semantic only), Detect(P3,P4,P5)",
    },
    "s3_scaf_p3": {
        "yaml": "yolov13_scaf_p3.yaml",
        "structure": "FAAR-B3 + SCAFFuse(P4->P3 detail only), Detect(P3,P4,P5)",
    },
}
...
def load_seed_metrics(root, run_id, stage, seed):
    path = root / "runs/test" / f"scaf_{run_id}_{stage}_seed{seed}" / "summary_metrics.json"
...
def main():
    ...
    for stage in args.stages:
        ...
        (test_dir / f"scaf_{args.run_id}_{stage}_summary.json").write_text(
            json.dumps(payload, indent=2), encoding="utf-8"
        )

    csv_path = test_dir / f"scaf_{args.run_id}_ablation.csv"
    ...
    (test_dir / f"scaf_{args.run_id}_summary.json").write_text(json.dumps(combined, indent=2), encoding="utf-8")
