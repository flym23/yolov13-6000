#!/usr/bin/env python3
"""Collect WSDR results using the exact D0 stage-summary JSON schema."""
...
STAGE_CONFIGS = {
    "w2_fixed": {
        "yaml": "yolov13_wsdr_fixed.yaml",
        "structure": "P5->P4 WSDRFuse fixed Haar LL injection; LH/HL/HH identity; P4->P3 FAARUp detail",
    },
    "w3_main": {
        "yaml": "yolov13_wsdr.yaml",
        "structure": "P5->P4 WSDRFuse adaptive Haar LL injection with HF-energy guidance; LH/HL/HH identity; P4->P3 FAARUp detail",
    },
    "w4_no_hf": {
        "yaml": "yolov13_wsdr_no_hf.yaml",
        "structure": "W3 without HF-energy gate input; Haar LH/HL/HH identity",
    },
    "w5_avgpool": {
        "yaml": "yolov13_wsdr_avgpool.yaml",
        "structure": "W3 with AvgPool+residual decomposition instead of Haar",
    },
    "w6_hf_reweight": {
        "yaml": "yolov13_wsdr_hf_reweight.yaml",
        "structure": "W3 with learnable Haar LH/HL/HH reweighting ablation",
    },
}
...
def load_seed_metrics(root, run_id, stage, seed):
    path = root / "runs/test" / f"wsdr_{run_id}_{stage}_seed{seed}" / "summary_metrics.json"
...
def main():
    ...
    for stage in args.stages:
        ...
        (test_dir / f"wsdr_{args.run_id}_{stage}_summary.json").write_text(
            json.dumps(payload, indent=2), encoding="utf-8"
        )

    csv_path = test_dir / f"wsdr_{args.run_id}_ablation.csv"
    ...
    (test_dir / f"wsdr_{args.run_id}_summary.json").write_text(
