...
STAGE_CONFIGS = {
    "a1_main": {
        "yaml": "yolov13-dcra.yaml",
        "structure": "P5->P4 DCRAUp(k=3,tau=0.10,entropy,lateral-guided); P4->P3 original nearest",
    },
    "a2_no_entropy": {
        "yaml": "yolov13-dcra-no-entropy.yaml",
        "structure": "A1 without entropy confidence; all other DCRA and original neck settings unchanged",
    },
    "a3_deep_only": {
        "yaml": "yolov13-dcra-deep-only.yaml",
        "structure": "A1 without lateral query guidance; deep nearest center-key query; P4->P3 original nearest",
    },
    "a4_tau020": {
    ...
    "a5_k5": {
        "yaml": "yolov13-dcra-k5.yaml",
        "structure": "A1 with discrete neighborhood 3x3->5x5 (9->25 candidates); all other settings unchanged",
    },
}
...
def load_seed_metrics(root, run_id, stage, seed):
    path = root / "runs/test" / f"dcra_{run_id}_{stage}_seed{seed}" / "summary_metrics.json"
...
def main():
    ...
    for stage in args.stages:
        ...
        (test_dir / f"dcra_{args.run_id}_{stage}_summary.json").write_text(
            json.dumps(payload, indent=2), encoding="utf-8"
        )

    csv_path = test_dir / f"dcra_{args.run_id}_ablation.csv"
    ...
    (test_dir / f"dcra_{args.run_id}_summary.json").write_text(
