#!/usr/bin/env python3
"""Collect UFCR-Next full/ablation validation summaries into JSON and CSV files."""
...
def load_seed_metrics(root, run_id, stage, seed):
    path = root / "runs/test" / f"ufcr_next_{run_id}_{stage}_seed{seed}" / "summary_metrics.json"
    data = json.loads(path.read_text(encoding="utf-8"))
    row = {"stage": stage, "seed": seed, "summary_path": str(path)}
...
def main():
    ...
    for stage in args.stages:
        ...
        (test_dir / f"ufcr_next_{args.run_id}_{stage}_summary.json").write_text(
            json.dumps(payload, indent=2), encoding="utf-8"
        )

    csv_path = test_dir / f"ufcr_next_{args.run_id}_ablation.csv"
    fieldnames = ["stage", "seed", *METRIC_KEYS.keys(), "summary_path"]
    ...
    (test_dir / f"ufcr_next_{args.run_id}_summary.json").write_text(json.dumps(combined, indent=2), encoding="utf-8")
