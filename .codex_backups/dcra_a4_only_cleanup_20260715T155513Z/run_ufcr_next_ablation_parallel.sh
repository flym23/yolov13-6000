...
STATE_DIR="$ROOT/runs/ufcr_next_ablation"
RUN_ID="${UFCR_NEXT_RUN_ID:-$(date +%Y%m%d_%H%M%S)}"
...
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
  echo "UFCR-Next chain lock already exists: $LOCK_DIR" >&2
  exit 73
fi
...
write_state() {
  ...
  local stage="$2"
  local code="${3:-0}"
  UFCR_NEXT_STATUS="$status" UFCR_NEXT_STAGE="$stage" UFCR_NEXT_CODE="$code" UFCR_NEXT_RUN_ID="$RUN_ID" \
    UFCR_NEXT_LAUNCHER_PID="$$" UFCR_NEXT_STATE_DIR="$STATE_DIR" "$PY" - <<'PY'
import json
import os
from datetime import datetime, timezone
from pathlib import Path

path = Path(os.environ["UFCR_NEXT_STATE_DIR"]) / "state.json"
payload = {
    "run_id": os.environ["UFCR_NEXT_RUN_ID"],
    "status": os.environ["UFCR_NEXT_STATUS"],
    "stage": os.environ["UFCR_NEXT_STAGE"],
    "exit_code": int(os.environ["UFCR_NEXT_CODE"]),
    "launcher_pid": int(os.environ["UFCR_NEXT_LAUNCHER_PID"]),
    "updated_at": datetime.now(timezone.utc).isoformat(),
    "stage_order": ["full", "wo_wt", "wo_edge", "wo_aarup", "wo_aardown"],
}
...
run_train() {
  ...
  local gpu="$2"
  local seed="$3"
  local name="ufcr_next_${RUN_ID}_${stage}_seed${seed}"
  local log="$TRAIN_DIR/${name}.log"

  CUDA_VISIBLE_DEVICES="$gpu" WANDB_DISABLED=true "$PY" "$ROOT/tools/train_ufcr_next_worker.py" \
...
run_group() {
  ...
  "$PY" "$ROOT/tools/collect_ufcr_next_ablation.py" --root "$ROOT" --run-id "$RUN_ID" --stages "$stage"
  write_state "stage_complete" "$stage"
}

STAGES=()
for stage in full wo_wt wo_edge wo_aarup wo_aardown; do
  run_group "$stage"
  STAGES+=("$stage")
done
...
write_state "running" "$CURRENT_STAGE"
"$PY" "$ROOT/tools/collect_ufcr_next_ablation.py" --root "$ROOT" --run-id "$RUN_ID" --stages "${STAGES[@]}"

CURRENT_STAGE="complete"
write_state "complete" "$CURRENT_STAGE"
echo "UFCR-Next ablation chain completed: run_id=$RUN_ID stages=${STAGES[*]}"
