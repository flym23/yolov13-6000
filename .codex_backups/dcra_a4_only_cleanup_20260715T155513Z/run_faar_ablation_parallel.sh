...
STATE_DIR="$ROOT/runs/faar_ablation"
RUN_ID="${FAAR_RUN_ID:-$(date +%Y%m%d_%H%M%S)}"
...
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
  echo "FAAR chain lock already exists: $LOCK_DIR" >&2
  exit 73
fi
...
write_state() {
  ...
  local stage="$2"
  local code="${3:-0}"
  FAAR_STATUS="$status" FAAR_STAGE="$stage" FAAR_CODE="$code" FAAR_RUN_ID="$RUN_ID" \
    FAAR_LAUNCHER_PID="$$" FAAR_STATE_DIR="$STATE_DIR" "$PY" - <<'PY'
import json
import os
from datetime import datetime, timezone
from pathlib import Path

path = Path(os.environ["FAAR_STATE_DIR"]) / "state.json"
payload = {
    "run_id": os.environ["FAAR_RUN_ID"],
    "status": os.environ["FAAR_STATUS"],
    "stage": os.environ["FAAR_STAGE"],
    "exit_code": int(os.environ["FAAR_CODE"]),
    "launcher_pid": int(os.environ["FAAR_LAUNCHER_PID"]),
    "updated_at": datetime.now(timezone.utc).isoformat(),
    "stage_order": ["b3_scale_specific", "b2_shared"],
}
...
run_train() {
  ...
  local gpu="$2"
  local seed="$3"
  local name="faar_${RUN_ID}_${stage}_seed${seed}"
  local log="$TRAIN_DIR/${name}.log"

  CUDA_VISIBLE_DEVICES="$gpu" WANDB_DISABLED=true "$PY" "$ROOT/tools/train_faar_worker.py" \
...
run_group() {
  ...
  "$PY" "$ROOT/tools/collect_faar_ablation.py" --root "$ROOT" --run-id "$RUN_ID" --stages "$stage"
  write_state "stage_complete" "$stage"
}

STAGES=()
for stage in b3_scale_specific b2_shared; do
  run_group "$stage"
  STAGES+=("$stage")
done
...
write_state "running" "$CURRENT_STAGE"
"$PY" "$ROOT/tools/collect_faar_ablation.py" --root "$ROOT" --run-id "$RUN_ID" --stages "${STAGES[@]}"

CURRENT_STAGE="complete"
write_state "complete" "$CURRENT_STAGE"
echo "FAAR ablation chain completed: run_id=$RUN_ID stages=${STAGES[*]}"
