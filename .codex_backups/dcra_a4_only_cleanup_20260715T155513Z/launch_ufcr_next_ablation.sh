...
PY
  ...
  if [[ "$status" == "running" || "$status" == "stage_complete" ]] && kill -0 "$pid" 2>/dev/null; then
    echo "UFCR-Next chain already active: pid=$pid"
    exit 0
  fi
fi
...
UFCR_NEXT_RUN_ID="$RUN_ID" nohup setsid "$ROOT/run_ufcr_next_ablation_parallel.sh" > "$LOG" 2>&1 < /dev/null &
pid=$!
echo "$pid" > "$STATE_DIR/launch.pid"
sleep 5
if ! kill -0 "$pid" 2>/dev/null; then
  echo "UFCR-Next launcher exited during startup. See $LOG" >&2
  tail -n 80 "$LOG" >&2 || true
  exit 1
fi
printf '{"run_id":"%s","launch_pid":%s,"log":"%s","stage_order":["full","wo_wt","wo_edge","wo_aarup","wo_aardown"]}\n' "$RUN_ID" "$pid" "$LOG" > "$STATE_DIR/launched.json"
echo "UFCR_NEXT_RUN_ID=$RUN_ID"
echo "UFCR_NEXT_LAUNCH_PID=$pid"
echo "UFCR_NEXT_LOG=$LOG"
