...
STAGE_ORDER=(r1_rp_scaf r4_rp_scaf_no_consistency r5_rp_scaf_channel r2_rp_scaf_a003 r3_rp_scaf_a008)

cd "$ROOT"
export PYTHONPATH="$ROOT"
mkdir -p "$TRAIN_DIR" "$TEST_DIR" "$STATE_DIR"
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
  echo "RP-SCAF chain lock already exists: $LOCK_DIR" >&2
  exit 73
fi
...
write_state() {
  ...
  local stage="$2"
  local code="${3:-0}"
  RP_SCAF_STATUS="$status" RP_SCAF_STAGE="$stage" RP_SCAF_CODE="$code" RP_SCAF_RUN_ID="$RUN_ID" \
    RP_SCAF_LAUNCHER_PID="$$" RP_SCAF_STATE_DIR="$STATE_DIR" "$PY" - <<'PY'
import json
import os
from datetime import datetime, timezone
from pathlib import Path

path = Path(os.environ["RP_SCAF_STATE_DIR"]) / "state.json"
payload = {
    "run_id": os.environ["RP_SCAF_RUN_ID"],
    "status": os.environ["RP_SCAF_STATUS"],
    "stage": os.environ["RP_SCAF_STAGE"],
    "exit_code": int(os.environ["RP_SCAF_CODE"]),
    "launcher_pid": int(os.environ["RP_SCAF_LAUNCHER_PID"]),
...
run_train() {
  ...
  local gpu="$2"
  local seed="$3"
  local name="rp_scaf_${RUN_ID}_${stage}_seed${seed}"
  local log="$TRAIN_DIR/${name}.log"

  CUDA_VISIBLE_DEVICES="$gpu" WANDB_DISABLED=true "$PY" "$ROOT/tools/train_rp_scaf_worker.py" \
...
run_group() {
  ...
  "$PY" "$ROOT/tools/collect_rp_scaf_ablation.py" --root "$ROOT" --run-id "$RUN_ID" --stages "$stage"
...
write_state "running" "$CURRENT_STAGE"
"$PY" "$ROOT/tools/collect_rp_scaf_ablation.py" --root "$ROOT" --run-id "$RUN_ID" --stages "${STAGES[@]}"

CURRENT_STAGE="complete"
write_state "complete" "$CURRENT_STAGE"
echo "RP-SCAF ablation chain completed: run_id=$RUN_ID stages=${STAGES[*]}"
