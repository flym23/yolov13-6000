...
from uw_common import (DATA, ROOT, SEEDS, TRAIN_ARGS, VARIANTS, WEIGHTS, model_yaml, now,
...
def wait_upstream(args, root):
    ...
    while True:
        ...
        if status in TERMINAL:
            ...
            os.execv('/usr/bin/bash', ['bash', str(ROOT / 'scripts/run_yolo11uw.sh'),
...
def run_workers(args, root, mode, seeds):
    ...
    try:
        for seed in seeds:
            ...
            command = [sys.executable, '-u', str(ROOT / 'tools/uw_worker.py'), mode,
...
def compare_all(args, root):
    rows = []
    baseline_file = run_root('A0', args.run_id) / 'test/AllSeed_summary.json'
    baseline = read_json(baseline_file) if baseline_file.is_file() else None
    ...
    write_json(root / 'test/Ablation_comparison.json', dict(variants=rows,
        caveat='Three seeds are descriptive evidence; validation and test share images/test. No independent test claim.'))
...
def run(args, root):
    ...
    with (root / 'manager.lock').open('a') as lock:
        ...
        try:
            ...
            if args.resume and metadata_file.is_file() and read_json(metadata_file)['sha256'] != contract:
