...
for stage_index in "${!STAGES[@]}"; do
  ...
  for seed in 0 1 2; do
    ...
    CUDA_VISIBLE_DEVICES=0 WANDB_DISABLED=true "$PY" "$ROOT/tools/train_medcra_worker.py" --root "$ROOT" --stage "$stage" --data "$DATA" --name "$name" --seed "$seed" --epochs 300 --patience 40 "${resume_args[@]}" > "$TRAIN/${name}.log" 2>&1 &
    pids+=("$!")
