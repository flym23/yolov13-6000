...
from tools import collect_wsdr_ablation


def test_wsdr_stage_summary_matches_d0_schema(tmp_path, monkeypatch):
    run_id = "schema_test"
    stage = "w3_main"
    for seed in (0, 1, 2):
        directory = tmp_path / "runs/test" / f"wsdr_{run_id}_{stage}_seed{seed}"
    ...
    collect_wsdr_ablation.main()
    result = json.loads(
        (tmp_path / "runs/test" / f"wsdr_{run_id}_{stage}_summary.json").read_text(encoding="utf-8")
    )
    assert list(result) == [
        "run_id",
        "stage",
