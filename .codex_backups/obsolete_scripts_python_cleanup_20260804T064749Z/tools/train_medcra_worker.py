...
def prepare_relocated_resume_checkpoint(root: Path, data: Path, name: str) -> Path:
    ...
    relocated_args.update(
        {
            ...
            "name": name,
            "save_dir": str(root / "runs/train" / name),
            "exist_ok": True,
            "epochs": 300,
            "patience": 40,
            "workers": 2,
    ...
    checkpoint_model = checkpoint.get("ema") or checkpoint.get("model")
    model_args = getattr(checkpoint_model, "args", None)
    if isinstance(model_args, dict):
        model_args.update(relocated_args)
    elif hasattr(model_args, "__dict__"):
        for key, value in relocated_args.items():
            setattr(model_args, key, value)
    else:
        raise TypeError(f"Checkpoint model arguments are invalid in {source}.")
