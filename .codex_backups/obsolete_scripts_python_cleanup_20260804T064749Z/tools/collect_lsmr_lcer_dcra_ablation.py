#!/usr/bin/env python3
"""Aggregate R1--R3 LSMR-LCER-DCRA results, paired endpoint statistics, and reused baselines."""
...
from lsmr_lcer_dcra_experiments import REFERENCE_BASELINES, STAGE_ORDER, STRUCTURES  # noqa: E402
...
def paired_statistics(control: list[dict], candidate: list[dict]) -> dict[str, dict]:
    ...
    if set(control_by_seed) != {0, 1, 2} or set(candidate_by_seed) != {0, 1, 2}:
        raise ValueError("paired LSMR analysis requires exactly seeds 0, 1, and 2 for R1 and each candidate")
    result = {}
...
def main() -> None:
    ...
    summary = {
        ...
        "comparison_rule": "Only R2/R3 versus same-batch R1 are paired; reused nearest baselines are unpaired historical references.",
    ...
    for stage in args.stages:
        ...
        if stage != "r1_matched_endpoint" and "r1_matched_endpoint" in args.stages:
            stage_summary["paired_vs_r1"] = paired_statistics(
                [row for row in records if row["stage"] == "r1_matched_endpoint"], stage_rows
            )
