...
class AGDSBAdaHyperedgeGen(AdaHyperedgeGen):
    """AdaHyperedgeGen variant returning unnormalized participation logits."""

    def forward(self, x):
        if x.ndim != 3:
            raise ValueError(f"AGDSBAdaHyperedgeGen expects [B, N, D], got {tuple(x.shape)}.")

        b, n, d = x.shape
        expected_dim = self.prototype_base.shape[1]
        if d != expected_dim:
            raise ValueError(
                "AGDSBAdaHyperedgeGen feature dimension mismatch: "
                f"input D={d}, expected D={expected_dim}."
            )
...
class AGDSBAdaHGConv(nn.Module):
    """AG-DSB-Lite with head-aligned, straight-through bounded adaptive gates.

    Hypergraph connectivity remains shared after the original multi-head logits
    are averaged by :class:`AGDSBAdaHyperedgeGen`. In learnable mode, one gate
    controls each contiguous channel group aligned with that head partition.
    Fixed mode keeps one scalar gate shared by every channel.
    """

    GATE_MODE = "head_aligned_direct_ste_v1"
    ...
    def __init__(
        ...
        self.learnable_eta = bool(learnable_eta)
        self.eps = float(eps)
        self.gate_mode = self.GATE_MODE

        if self.embed_dim <= 0:
            raise ValueError(f"embed_dim must be positive, got {self.embed_dim}.")
        if self.num_hyperedges <= 1:
            raise ValueError(f"num_hyperedges must be greater than 1, got {self.num_hyperedges}.")
        if self.num_heads <= 0:
        ...
        if self.topk != 0 and not (1 <= self.topk < self.num_hyperedges):
            raise ValueError(
                "topk must be 0 or satisfy 1 <= topk < num_hyperedges, got "
                f"topk={self.topk}, num_hyperedges={self.num_hyperedges}."
            )
        if not (0.0 <= self.eta_min < self.eta_init < self.eta_max):
            raise ValueError(
                "Require 0 <= eta_min < eta_init < eta_max, got "
                f"{self.eta_min}, {self.eta_init}, {self.eta_max}."
            )
        if not (0.0 < self.delta_scale_max <= 1.0):
            raise ValueError(
                "delta_scale_max must satisfy 0 < delta_scale_max <= 1.0, "
                f"got {self.delta_scale_max}."
            )
        if self.eps <= 0.0:
            raise ValueError(f"eps must be positive, got {self.eps}.")

        self.edge_generator = AGDSBAdaHyperedgeGen(
        ...
        self.edge_proj = nn.Sequential(nn.Linear(self.embed_dim, self.embed_dim), nn.GELU())
        self.node_proj = nn.Sequential(nn.Linear(self.embed_dim, self.embed_dim), nn.GELU())

        if self.learnable_eta:
            # "bias" keeps the direct FP32 gate in the optimizer's no-decay group.
            self.eta_head_bias = nn.Parameter(
                torch.full((self.num_heads,), self.eta_init, dtype=torch.float32)
            )
        else:
            self.register_buffer(
                "eta_fixed", torch.tensor([self.eta_init], dtype=torch.float32), persistent=True
            )

    def effective_eta(self, dtype=None, device=None):
        """Return bounded gates: ``[num_heads]`` learnable or ``[1]`` fixed."""
        if self.learnable_eta:
            raw_fp32 = self.eta_head_bias.float()
            bounded_fp32 = raw_fp32.clamp(min=self.eta_min, max=self.eta_max)
            # Forward equals clamp(raw), while d(eta)/d(raw) remains exactly 1.
            eta_fp32 = raw_fp32 + (bounded_fp32 - raw_fp32).detach()
        else:
            eta_fp32 = self.eta_fixed.float()
        if device is not None:
            eta_fp32 = eta_fp32.to(device=device)
        if dtype is not None:
            eta_fp32 = eta_fp32.to(dtype=dtype)
        return eta_fp32

    def _dense_v2e_weights(self, logits):
        """Dense vertex-to-hyperedge normalization over the vertex dimension."""
        return torch.softmax(logits.float(), dim=1).to(dtype=logits.dtype)

    def _build_e2v_weights(self, logits):
        """Independently normalized dense or exact top-k sparse E->V weights."""
        if logits.ndim != 3:
            raise ValueError(f"logits must have shape [B, N, E], got {tuple(logits.shape)}.")
        if logits.shape[-1] != self.num_hyperedges:
            raise ValueError(
                f"Hyperedge count mismatch: input E={logits.shape[-1]}, expected E={self.num_hyperedges}."
            )
        logits_fp32 = logits.float()
        if self.topk == 0:
            return torch.softmax(logits_fp32, dim=2).to(dtype=logits.dtype)
        topk_values, topk_indices = torch.topk(
            logits_fp32, k=self.topk, dim=2, largest=True, sorted=False
        )
        topk_weights = torch.softmax(topk_values, dim=2).to(dtype=logits.dtype)
        weights = torch.zeros_like(logits)
        weights.scatter_(dim=2, index=topk_indices, src=topk_weights)
        return weights

    def _normalize_message_delta(self, message_base, message_delta):
        """Bound message-space DSB residual RMS by base-message RMS."""
        if not self.normalize_delta:
            return message_delta
        if message_base.shape != message_delta.shape:
            raise ValueError(
                "message_base and message_delta must have identical shapes, "
                f"got {tuple(message_base.shape)} and {tuple(message_delta.shape)}."
            )
        base_power = message_base.float().square().mean(dim=-1, keepdim=True)
        delta_power = message_delta.float().square().mean(dim=-1, keepdim=True)
        # The numerator intentionally excludes eps. This gives a strict
        # energy upper bound even when the base message is near zero.
        scale_to_base = torch.sqrt(base_power / (delta_power + self.eps))
        scale = scale_to_base.detach().clamp(max=min(self.delta_scale_max, 1.0))
        return message_delta * scale.to(dtype=message_delta.dtype)

    def _apply_head_aligned_eta(self, message_delta):
        """Apply one scalar fixed gate or one learnable gate per channel head."""
        if message_delta.ndim != 3:
            raise ValueError(
                f"message_delta must have shape [B, N, D], got {tuple(message_delta.shape)}."
            )
        b, n, d = message_delta.shape
        if d != self.embed_dim:
            raise ValueError(
                f"message_delta feature dimension mismatch: input D={d}, expected D={self.embed_dim}."
            )

        eta = self.effective_eta(dtype=message_delta.dtype, device=message_delta.device)
        if eta.numel() == 1:
            return message_delta * eta.reshape(1, 1, 1)
        if eta.numel() != self.num_heads:
            raise RuntimeError(
                "Learnable eta count must equal num_heads, "
                f"got eta_count={eta.numel()}, num_heads={self.num_heads}."
            )
        delta_heads = message_delta.reshape(b, n, self.num_heads, self.head_dim)
        gated_heads = delta_heads * eta.reshape(1, 1, self.num_heads, 1)
        return gated_heads.reshape(b, n, d)

    def forward(self, x):
        if x.ndim != 3:
            raise ValueError(f"AGDSBAdaHGConv expects [B, N, D], got {tuple(x.shape)}.")
        if x.shape[-1] != self.embed_dim:
            raise ValueError(
                f"Feature dimension mismatch: input D={x.shape[-1]}, expected D={self.embed_dim}."
            )
        logits = self.edge_generator(x)
        a_v2e = self._dense_v2e_weights(logits)
        edge_feature = self.edge_proj(torch.bmm(a_v2e.transpose(1, 2), x))
        message_base = torch.bmm(a_v2e, edge_feature)
        a_e2v = self._build_e2v_weights(logits)
        message_dsb = torch.bmm(a_e2v, edge_feature)
        message_delta = self._normalize_message_delta(message_base, message_dsb - message_base)
        gated_delta = self._apply_head_aligned_eta(message_delta)
        mixed_message = message_base + gated_delta
        node_update = self.node_proj(mixed_message)
...
class DSBAdaHyperedgeGen(AdaHyperedgeGen):
    """AdaHyperedgeGen variant that returns unnormalized participation logits."""

    def forward(self, x):
        if x.ndim != 3:
            raise ValueError(f"DSBAdaHyperedgeGen expects [B, N, D], got shape {tuple(x.shape)}.")

        b, n, d = x.shape
        if d != self.prototype_base.shape[1]:
            raise ValueError(
                "DSBAdaHyperedgeGen feature dimension mismatch: "
                f"input D={d}, expected {self.prototype_base.shape[1]}."
            )

        if self.context == "mean":
            context_cat = x.mean(dim=1)
        elif self.context == "max":
            context_cat = x.max(dim=1).values
        else:
            avg_context = x.mean(dim=1)
            max_context = x.max(dim=1).values
            context_cat = torch.cat((avg_context, max_context), dim=-1)
...
class DSBAdaHGConv(nn.Module):
    """Dense-collection and sparse-broadcast adaptive hypergraph convolution."""
    ...
    def __init__(
        ...
        self.topk = int(topk)
        self.eta_max = float(eta_max)
        self.use_dual_norm = bool(use_dual_norm)
        if self.embed_dim <= 0:
        ...
        self.edge_generator = DSBAdaHyperedgeGen(
            node_dim=self.embed_dim,
            num_hyperedges=self.num_hyperedges,
            num_heads=num_heads,
            dropout=dropout,
        ...
        self.edge_proj = nn.Sequential(nn.Linear(self.embed_dim, self.embed_dim), nn.GELU())
        self.node_proj = nn.Sequential(nn.Linear(self.embed_dim, self.embed_dim), nn.GELU())
        self.eta_raw = nn.Parameter(torch.zeros(1, dtype=torch.float32))

    def _build_e2v_weights(self, logits):
        """Build independently normalized, optionally top-k sparse E->V weights."""
        if logits.ndim != 3 or logits.shape[-1] != self.num_hyperedges:
            raise ValueError(
                "DSBAdaHGConv logits must be [B, N, E] with "
                f"E={self.num_hyperedges}, got {tuple(logits.shape)}."
            )
        if not self.use_dual_norm:
            return torch.softmax(logits, dim=1)

        effective_topk = min(max(self.topk, 0), self.num_hyperedges)
        if effective_topk <= 0 or effective_topk >= self.num_hyperedges:
            return torch.softmax(logits, dim=2)

        topk_indices = torch.topk(
            logits, k=effective_topk, dim=2, largest=True, sorted=False
        ).indices
        sparse_mask = torch.zeros_like(logits, dtype=torch.bool)
        sparse_mask.scatter_(dim=2, index=topk_indices, value=True)
        sparse_logits = logits.masked_fill(~sparse_mask, torch.finfo(logits.dtype).min)
        return torch.softmax(sparse_logits, dim=2)

    def forward(self, x):
        if x.ndim != 3:
            raise ValueError(f"DSBAdaHGConv expects [B, N, D], got shape {tuple(x.shape)}.")
        if x.shape[-1] != self.embed_dim:
            raise ValueError(
                f"DSBAdaHGConv feature dimension mismatch: input D={x.shape[-1]}, expected {self.embed_dim}."
            )

        logits = self.edge_generator(x)
        a_v2e = torch.softmax(logits, dim=1)
        edge_feature = torch.bmm(a_v2e.transpose(1, 2), x)
        edge_feature = self.edge_proj(edge_feature)
        message_base = torch.bmm(a_v2e, edge_feature)
        a_e2v = self._build_e2v_weights(logits)
        message_dsb = torch.bmm(a_e2v, edge_feature)
        eta = self.eta_max * torch.tanh(self.eta_raw)
        message = message_base + eta * (message_dsb - message_base)
        return x + self.node_proj(message)


class DSBAdaHGComputation(nn.Module):
    """4D feature-map wrapper for DSBAdaHGConv."""
    ...
    def forward(self, x):
        if x.ndim != 4:
            raise ValueError(f"DSBAdaHGComputation expects [B, C, H, W], got shape {tuple(x.shape)}.")
        b, c, h, w = x.shape
        if c != self.embed_dim:
            raise ValueError(
                f"DSBAdaHGComputation channel mismatch: input C={c}, expected {self.embed_dim}."
            )
        tokens = self.hgnn(x.flatten(2).transpose(1, 2))
        return tokens.transpose(1, 2).reshape(b, c, h, w)


class DSBC3AH(nn.Module):
    """C3AH variant using DSBAdaHGComputation."""
    ...
    def __init__(
        ...
        hidden = int(c2 * e)
        if hidden % 16 != 0:
            raise ValueError(f"DSBC3AH hidden dimension must be a multiple of 16, got {hidden}.")
        num_heads = hidden // 16
        self.cv1 = Conv(c1, hidden, 1, 1)
        self.cv2 = Conv(c1, hidden, 1, 1)
        self.m = DSBAdaHGComputation(
            embed_dim=hidden,
            num_hyperedges=num_hyperedges,
            num_heads=num_heads,
            dropout=0.1,
...
class AGDSBHyperACE(HyperACE):
    """HyperACE with AG-DSB-Lite head-aligned adaptive gates."""
    ...
    def __init__(
        ...
        if dsb_branches not in {1, 2}:
            raise ValueError(
                "dsb_branches must be 1 or 2, "
                f"got {dsb_branches}."
            )
        ...
        def replace_hgnn(
            ...
            if not hasattr(
                ...
                raise AttributeError(
                    "C3AH branch has no attribute 'm'."
                )
            if not hasattr(
                branch.m,
                "hgnn",
                ...
                raise AttributeError(
                    "C3AH.m has no attribute 'hgnn'."
                )
            old_hgnn = branch.m.hgnn
            if not isinstance(
                old_hgnn,
                AdaHGConv,
            ):
                raise TypeError(
                    "Expected branch.m.hgnn to be AdaHGConv, "
                    f"got {type(old_hgnn).__name__}."
                )

            old_generator = (
                old_hgnn.edge_generator
            )
            embed_dim = (
                old_hgnn.edge_proj[0]
                .in_features
            )
            old_num_hyperedges = (
                old_generator.num_hyperedges
            )
            old_num_heads = (
                old_generator.num_heads
            )
            old_context = (
                old_generator.context
            )
            old_dropout = (
                old_generator.dropout.p
            )
            ...
            with torch.random.fork_rng(
                ...
                new_hgnn = AGDSBAdaHGConv(
                    embed_dim=embed_dim,
                    num_hyperedges=old_num_hyperedges,
            ...
            if (
                ...
                raise RuntimeError(
                    "Unexpected missing keys while transferring "
                    "AdaHGConv weights: "
                    f"{load_result.missing_keys}."
                )
            if load_result.unexpected_keys:
                raise RuntimeError(
                    "Unexpected keys while transferring AdaHGConv "
                    "weights: "
...
class DSBHyperACE(HyperACE):
    """HyperACE with one or two dense-collection, sparse-broadcast high-order branches."""
    ...
    def __init__(
        ...
        dsb_branches = int(dsb_branches)
        if dsb_branches not in {1, 2}:
            raise ValueError("dsb_branches must be 1 or 2.")

        def build_dsb_branch(original_branch):
            branch_c1 = original_branch.cv1.conv.in_channels
            branch_hidden = original_branch.cv1.conv.out_channels
            branch_c2 = original_branch.cv3.conv.out_channels
            return DSBC3AH(
                c1=branch_c1,
                c2=branch_c2,
                e=float(branch_hidden) / float(branch_c2),
                num_hyperedges=num_hyperedges,
                context=context,
                topk=dsb_topk,
                eta_max=eta_max,
        ...
        if dsb_branches == 2:
            self.branch1 = build_dsb_branch(self.branch1)
        self.branch2 = build_dsb_branch(self.branch2)
