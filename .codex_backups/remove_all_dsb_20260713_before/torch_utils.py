...
def capture_fp32_parameters(model: nn.Module, suffixes: Tuple[str, ...]) -> Dict[str, torch.Tensor]:
    """Clone selected floating-point parameters in FP32 before a whole-model precision conversion."""
    return {
        name: parameter.detach().float().clone()
        for name, parameter in model.named_parameters()
        if parameter.is_floating_point() and name.endswith(suffixes)
    }


def restore_fp32_parameters(model: nn.Module, state: Dict[str, torch.Tensor]) -> None:
    """Restore selected parameters as FP32 without changing their Parameter identities."""
    parameters = dict(model.named_parameters())
    missing = state.keys() - parameters.keys()
    if missing:
        raise KeyError(f"Parameters missing while restoring FP32 state: {sorted(missing)}")
    with torch.no_grad():
...
def strip_optimizer(f: Union[str, Path] = "best.pt", s: str = "", updates: dict = None) -> dict:
    ...
    fp32_controls = capture_fp32_parameters(x["model"], suffixes=("eta_bias", "eta_head_bias"))
    x["model"].half()  # to FP16
    restore_fp32_parameters(x["model"], fp32_controls)
...
class EarlyStopping:
    ...
    def __call__(self, epoch, fitness):
        ...
        if fitness > self.best_fitness:  # require strict improvement so unchanged mAP counts toward patience
