...
from common import (BASELINE, DATA, ROOT, SEEDS, TRAIN_ARGS, VARIANTS, WEIGHTS, model_yaml, now,
...
def wait_upstream(args, root):
    ...
    while True:
        ...
        if status in TERMINAL:
            ...
            os.execv('/usr/bin/bash', ['bash', str(ROOT / 'scripts/run_yolo11uw_v2.sh'),
...
def run_workers(args, root, mode, seeds):
    ...
    try:
        for seed in seeds:
            log = root / mode / f'seed{seed}_worker.log'
            handle = log.open('a', encoding='utf-8', buffering=1)
            handles.append(handle)
            command = [sys.executable, '-u', str(ROOT / 'tools/uwv2/worker.py'), mode,
...
def compare_all(args, root):
    rows = []
    baseline_file = BASELINE / 'test/AllSeed_summary.json'
    baseline = read_json(baseline_file)
    write_json(root / 'test/A0_reused_baseline.json', baseline)
    ...
    write_json(root / 'test/Ablation_comparison.json', dict(variants=rows,
        ...
        baseline_policy='User requested reuse of A0; no new B0 training.',
        caveat='A0: epochs=300/patience=30; v2: epochs=180/patience=0. Training schedules differ. '
               'Three seeds are descriptive evidence; val and test share images/test, not independent testing.'))


def run(args, root):
    if args.resume:
        raise ValueError('UW v2 resume is disabled; use a new immutable run_id for a fresh attempt')
    ...
    with (root / 'manager.lock').open('a') as lock:
        ...
        try:
            files = required_files(args.variant) + [BASELINE / 'test/AllSeed_summary.json']
