...
from common import DATA, ROOT, TRAIN_ARGS, WEIGHTS, model_yaml, read_json, runtime, sha256, write_json
...
def train(args):
    if args.resume:
        raise ValueError('UW v2 requires an uninterrupted run; WIoU resume is not validated')
    runtime()
    ...
    if len(rows) != TRAIN_ARGS['epochs'] or int(rows[-1]['epoch']) != TRAIN_ARGS['epochs']:
        raise RuntimeError(f'Expected exactly 180 epochs, received {len(rows)} rows')
...
def main():
    ...
    parser.add_argument('--variant', choices=['B1', 'B2', 'B3', 'B4', 'B5'], required=True)
