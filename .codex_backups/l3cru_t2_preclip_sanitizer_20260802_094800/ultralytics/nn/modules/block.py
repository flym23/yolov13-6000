...
class DDFCalib(nn.Module):
    """Degradation-decoupled low-frequency and detail calibration for underwater backbone features."""

    def __init__(self, c1, c2=None, reduction=8, context_kernel=3, max_gain=0.10, eps=1e-6):
        super().__init__()
        self.c1 = int(c1)
        ...
        if self.c1 <= 0 or self.c2 <= 0:
            raise ValueError(f"DDFCalib channels must be positive, got {self.c1}->{self.c2}.")
        if self.reduction <= 0:
            raise ValueError(f"reduction must be positive, got {self.reduction}.")
        if self.context_kernel < 1 or self.context_kernel % 2 == 0:
            raise ValueError(f"context_kernel must be a positive odd integer, got {self.context_kernel}.")
        if not 0.0 < self.max_gain <= 1.0:
            raise ValueError(f"max_gain must satisfy 0 < gain <= 1, got {self.max_gain}.")
        if self.eps <= 0.0:
            raise ValueError(f"eps must be positive, got {self.eps}.")

        self.proj = nn.Identity() if self.c1 == self.c2 else nn.Conv2d(self.c1, self.c2, 1, bias=False)
        hidden = max(self.c2 // self.reduction, 16)
        # No BatchNorm is used here, so pooled [B, C, 1, 1] statistics remain valid for a final batch of one image.
        self.stat_gate = nn.Sequential(
            nn.Conv2d(3 * self.c2, hidden, 1, bias=True),
            nn.SiLU(inplace=True),
            nn.Conv2d(hidden, 2 * self.c2, 1, bias=True),
        )
        self.low_branch = nn.Sequential(
            nn.Conv2d(
                ...
                self.context_kernel,
                1,
                self.context_kernel // 2,
                groups=self.c2,
                bias=False,
            ),
            nn.SiLU(inplace=True),
            nn.Conv2d(self.c2, self.c2, 1, bias=True),
        )
        ...
        # The first optimization step opens the two residual paths; branch weights receive signal afterwards.
        self.low_scale = nn.Parameter(torch.zeros(1, dtype=torch.float32))
    ...
    def _replicate_avg_pool2d(x, kernel_size):
        ...
        pad = kernel_size // 2
        return F.avg_pool2d(
            F.pad(x, (pad, pad, pad, pad), mode="replicate"),
    ...
    def forward(self, x):
        if x.ndim != 4:
            raise ValueError(f"DDFCalib expects a 4D NCHW tensor, got {tuple(x.shape)}.")
        if x.shape[1] != self.c1:
            raise ValueError(f"DDFCalib channel mismatch: got {x.shape[1]}, expected {self.c1}.")

        x = self.proj(x)
        low = self._replicate_avg_pool2d(x, self.context_kernel)
        detail = x - low
        low_mean = low.mean(dim=(2, 3), keepdim=True)
        low_deviation = (low - low_mean).abs().mean(dim=(2, 3), keepdim=True)
        detail_rms = detail.float().square().mean(dim=(2, 3), keepdim=True).add(self.eps).sqrt()
        detail_abs = detail.abs().mean(dim=(2, 3), keepdim=True)
        stats = torch.cat((low_deviation, detail_rms.to(dtype=x.dtype), detail_abs), dim=1)
        low_gate, detail_gate = self.stat_gate(stats).sigmoid().chunk(2, dim=1)

        low_correction = self.low_branch(low - low_mean)
        detail_correction = self.detail_branch(detail)
        low_gain = self.max_gain * torch.tanh(self.low_scale).to(dtype=x.dtype)
        detail_gain = self.max_gain * torch.tanh(self.detail_scale).to(dtype=x.dtype)
        output = x + low_gain * low_gate * low_correction + detail_gain * detail_gate * detail_correction
        if output.shape != x.shape:
            raise RuntimeError(f"DDFCalib output shape mismatch: got {tuple(output.shape)}, expected {tuple(x.shape)}.")
        return output
...
class TIERDCRAUp(DCRAUp):
    ...
    def __init__(self, c_deep, c_lateral, config=None):
        ...
        if not isinstance(config, dict):
            raise TypeError(f"TIERDCRAUp config must be a dict or None, got {type(config).__name__}.")
        ...
        reliability_mode = str(cfg["reliability_mode"]).lower()
        margin_power = float(cfg["margin_power"])
        consensus_kernel = int(cfg["consensus_kernel"])
        detail_kernel = int(cfg["detail_kernel"])
        if reliability_mode not in self._VALID_RELIABILITY_MODES:
            raise ValueError(
                f"reliability_mode must be one of {sorted(self._VALID_RELIABILITY_MODES)}, got {reliability_mode!r}."
            )
        if margin_power <= 0.0:
        ...
        if detail_kernel < 1 or detail_kernel % 2 == 0:
            raise ValueError(f"detail_kernel must be a positive odd integer, got {detail_kernel}.")
    ...
    def _entropy_confidence(self, weights):
        if weights.ndim != 4 or weights.shape[1] != self.num_candidates:
            raise ValueError(
                "weights must have shape [B,K,H,W] with "
                f"K={self.num_candidates}, got {tuple(weights.shape)}."
            )
        if not self.use_entropy:
            return torch.ones_like(weights[:, :1], dtype=torch.float32)
    ...
    def _margin_confidence(self, weights):
        ...
        top2 = torch.topk(weights.float(), k=2, dim=1, largest=True, sorted=True).values
        return (top2[:, :1] - top2[:, 1:2]).clamp(0.0, 1.0).pow(self.margin_power)

    def _detail_guard(self, lateral, margin):
        if lateral.ndim != 4:
            raise ValueError(f"lateral must be 4D NCHW, got {tuple(lateral.shape)}.")
        if margin.ndim != 4 or margin.shape[1] != 1 or margin.shape[-2:] != lateral.shape[-2:]:
            raise ValueError(
                "margin must have shape [B,1,H,W] matching lateral, got "
                f"margin={tuple(margin.shape)}, lateral={tuple(lateral.shape)}."
            )
        lateral_fp32 = lateral.float()
        local_mean = self._replicate_avg_pool2d(lateral_fp32, 3)
        detail = (lateral_fp32 - local_mean).abs().mean(dim=1, keepdim=True)
        detail_reference = self._replicate_avg_pool2d(detail, self.detail_kernel)
        detail_ratio = detail / detail_reference.add(self.eps)
        detail_risk = detail_ratio / (1.0 + detail_ratio)
        detail_risk = torch.nan_to_num(detail_risk, nan=0.0, posinf=1.0, neginf=0.0).clamp(0.0, 1.0)
        return (1.0 - detail_risk * (1.0 - margin)).clamp(0.0, 1.0)

    def _compute_reliability(self, weights, lateral):
        entropy_conf = self._entropy_confidence(weights)
        if self.reliability_mode == "entropy":
            reliability = entropy_conf
        else:
            margin = self._margin_confidence(weights)
            joint = (entropy_conf * margin).clamp(0.0, 1.0).sqrt()
            local_mean = self._replicate_avg_pool2d(joint, self.consensus_kernel)
            consensus = (joint * local_mean).clamp(0.0, 1.0).sqrt()
            reliability = consensus if self.reliability_mode == "entropy_margin" else consensus * self._detail_guard(lateral, margin)
        ...
        if self.detach_reliability or self.detach_confidence:
    ...
    def _compute_alignment(self, deep, lateral):
        ...
        key_patches = self._extract_local_patches(key_low)
        value_patches = self._extract_local_patches(deep)
        exact_scale = target_size == (deep.shape[-2] * self.scale, deep.shape[-1] * self.scale)
        if exact_scale:
            reassembled_fp32, weights = self._phase_correlate_and_reassemble(query, key_patches, value_patches)
        else:
            reassembled_fp32, weights = self._fallback_correlate_and_reassemble(
                query, key_patches, value_patches, target_size
            )
        reassembled = reassembled_fp32.to(dtype=deep.dtype)
        reliability = self._compute_reliability(weights, lateral)
        raw_delta = reassembled - base
        if self.finite_fallback:
            # NaN * 0 remains NaN. Sanitize the candidate correction before gating
            # so each non-finite location exactly falls back to nearest-neighbor.
            raw_delta = torch.where(
                torch.isfinite(raw_delta),
                raw_delta,
                torch.zeros_like(raw_delta),
            )
        elif not torch.isfinite(raw_delta).all():
            raise RuntimeError("TIERDCRAUp reassembly delta contains non-finite values.")
        residual = raw_delta * reliability.to(device=deep.device, dtype=deep.dtype)
...
class SADIP3Fuse(nn.Module):
    ...
    def __init__(self, c_p2, c_p3, config=None):
        ...
        if config is None:
            config = {}
        if not isinstance(config, dict):
            raise TypeError(f"SADIP3Fuse config must be a dict or None, got {type(config).__name__}.")
        unknown = sorted(set(config) - set(self._DEFAULT_CONFIG))
        if unknown:
        ...
        if self.c_p2 <= 0 or self.c_p3 <= 0:
            raise ValueError(f"c_p2 and c_p3 must be positive, got {self.c_p2}, {self.c_p3}.")
        if self.reduction <= 0:
            raise ValueError(f"reduction must be positive, got {self.reduction}.")
        ...
        if self.agreement_power <= 0.0:
            raise ValueError(f"agreement_power must be positive, got {self.agreement_power}.")
        if self.support_power <= 0.0:
            raise ValueError(f"support_power must be positive, got {self.support_power}.")
        if not 0.0 < self.max_residual_ratio <= 1.0:
            raise ValueError(f"max_residual_ratio must satisfy 0 < ratio <= 1, got {self.max_residual_ratio}.")
        if self.eps <= 0.0:
            raise ValueError(f"eps must be positive, got {self.eps}.")
        ...
        with torch.random.fork_rng(devices=[], enabled=True):
            ...
            self.p2_proj = nn.Sequential(
                nn.Conv2d(self.c_p2, self.hidden, 1, bias=False), nn.BatchNorm2d(self.hidden), nn.SiLU(inplace=True)
            )
            self.p3_proj = nn.Sequential(
                nn.Conv2d(self.c_p3, self.hidden, 1, bias=False), nn.BatchNorm2d(self.hidden), nn.SiLU(inplace=True)
            )
            self.detail_refine = nn.Sequential(
                nn.Conv2d(
                    self.hidden, self.hidden, self.detail_kernel, 1, self.detail_kernel // 2,
                    groups=self.hidden, bias=False,
                ),
    ...
    def _anti_alias_downsample(self, x, target_size):
        ...
        filtered = F.conv2d(F.pad(x, (1, 1, 1, 1), mode="replicate"), weight, stride=2, groups=x.shape[1])
        if tuple(filtered.shape[-2:]) != tuple(target_size):
            if self.strict_scale:
                raise ValueError(
                    "SADIP3Fuse spatial scale mismatch after anti-aliased downsampling: "
    ...
    def _validate_inputs(self, p2, p3):
        if not isinstance(p2, torch.Tensor) or not isinstance(p3, torch.Tensor):
            raise TypeError("SADIP3Fuse expects tensor inputs [p2_feature, p3_feature].")
        if p2.ndim != 4 or p3.ndim != 4:
            raise ValueError(f"SADIP3Fuse expects 4D NCHW tensors, got {tuple(p2.shape)} and {tuple(p3.shape)}.")
        ...
        if p2.device != p3.device or p2.dtype != p3.dtype:
            raise ValueError(f"P2/P3 device or dtype mismatch: {p2.device}/{p2.dtype} vs {p3.device}/{p3.dtype}.")
        expected = (p3.shape[-2] * 2, p3.shape[-1] * 2)
        if self.strict_scale and tuple(p2.shape[-2:]) != expected:
            raise ValueError(
                "SADIP3Fuse requires P2 spatial size to be exactly 2x P3, "
                f"got P2={tuple(p2.shape[-2:])}, P3={tuple(p3.shape[-2:])}."
            )

    def _compute_gate(self, p2_embed, p3_embed):
        if p2_embed.shape != p3_embed.shape:
            raise ValueError(f"Embedded P2/P3 shape mismatch: {tuple(p2_embed.shape)} vs {tuple(p3_embed.shape)}.")
        p2_fp32, p3_fp32 = p2_embed.float(), p3_embed.float()
        ...
        detail = p2_fp32 - self._replicate_avg_pool2d(p2_fp32, self.detail_kernel)
        ...
        support = (salience * self._replicate_avg_pool2d(salience, self.support_kernel)).clamp(0.0, 1.0).sqrt()
        gate = (agreement * support.pow(self.support_power)).clamp(0.0, 1.0)
        if self.finite_fallback:
        ...
        return (gate.detach() if self.detach_gate else gate), detail.to(dtype=p2_embed.dtype)

    def _bound_correction(self, base, correction):
        correction_fp32 = correction.float()
        if self.finite_fallback:
            correction_fp32 = torch.nan_to_num(correction_fp32, nan=0.0, posinf=0.0, neginf=0.0)
        ...
        bounded = correction_fp32 * (bound.detach() if self.detach_bound else bound)
        if self.finite_fallback:
            bounded = torch.nan_to_num(bounded, nan=0.0, posinf=0.0, neginf=0.0)
        elif not torch.isfinite(bounded).all():
            raise RuntimeError("SADIP3Fuse bounded correction contains non-finite values.")
        return bounded

    def forward(self, x):
        if not isinstance(x, (list, tuple)) or len(x) != 2:
            raise TypeError("SADIP3Fuse expects input as [p2_feature, p3_feature].")
        p2, p3 = x
        self._validate_inputs(p2, p3)
        p2_down = self._anti_alias_downsample(p2, p3.shape[-2:])
        p2_embed, p3_embed = self.p2_proj(p2_down), self.p3_proj(p3)
        ...
        if self.detail_out.weight.dtype == torch.float32:
            ...
            correction = self.detail_out(refined.to(dtype=self.detail_out.weight.dtype)).float()
        output = p3.float() + self._bound_correction(p3, correction)
        output = output if p3.dtype == torch.float32 else output.to(dtype=p3.dtype)
        if tuple(output.shape) != tuple(p3.shape):
            raise RuntimeError(f"SADIP3Fuse output mismatch: got {tuple(output.shape)}, expected {tuple(p3.shape)}.")
        return output


class MEDCRAUp(DCRAUp):
    """Moment- and Energy-Constrained DCRA upsampling without extra parameters."""
    ...
    def __init__(
        ...
        if self.moment_scale_max < 1.0:
            raise ValueError(
                "moment_scale_max must be greater than or equal to 1, "
                f"got {self.moment_scale_max}."
            )

    @staticmethod
    def _spatial_mean_and_centered_rms(x, eps):
        """Return FP32 per-sample/channel spatial mean, centered map, and RMS."""
        if x.ndim != 4:
            raise ValueError(
                "Spatial statistics require a 4D NCHW tensor, "
                f"got shape {tuple(x.shape)}."
            )
        x_fp32 = x.float()
        mean = x_fp32.mean(dim=(2, 3), keepdim=True)
        centered = x_fp32 - mean
        rms = centered.square().mean(dim=(2, 3), keepdim=True).add(float(eps)).sqrt()
        return mean, centered, rms

    def _moment_preserving_residual(self, base, residual):
        """Match the confidence-blended candidate's channel moments to ``base``."""
        ...
        moment_scale = (base_rms / candidate_rms.clamp_min(self.eps)).clamp(max=self.moment_scale_max)
        if self.detach_moment_scale:
            moment_scale = moment_scale.detach()
        candidate_matched = candidate_centered * moment_scale + base_mean
        return (candidate_matched - base.float()).to(dtype=base.dtype)

    def _condition_output_correction(self, base, correction):
        """Center and optionally bound projected-correction RMS in FP32."""
        if base.shape != correction.shape:
            raise ValueError(
                "base and correction must have identical shapes, got "
                f"base={tuple(base.shape)}, correction={tuple(correction.shape)}."
            )

        correction_fp32 = correction.float()
        if self.center_correction:
            correction_fp32 = correction_fp32 - correction_fp32.mean(dim=(2, 3), keepdim=True)
        if not self.use_energy_bound:
            return correction_fp32
        ...
        allowed_rms = self.max_residual_ratio * base_rms
        energy_scale = torch.minimum(
            torch.ones_like(correction_rms),
            allowed_rms / correction_rms.clamp_min(self.eps),
        )
        if self.detach_energy_scale:
            energy_scale = energy_scale.detach()
        return correction_fp32 * energy_scale

    def forward(self, x):
        if not isinstance(x, (list, tuple)) or len(x) != 2:
            raise TypeError("MEDCRAUp expects input as [deep_feature, lateral_feature].")

        deep, lateral = x
        base, residual, _, _ = self._compute_alignment(deep, lateral)
        residual = self._moment_preserving_residual(base, residual)

        # Preserve DCRAUp's FP32 projection path so all constraints disabled is bitwise A4-equivalent.
        if self.residual_out.weight.dtype == torch.float32:
            with torch.autocast(device_type=deep.device.type, enabled=False):
                correction = self.residual_out(residual.float())
        else:
            correction = self.residual_out(residual.to(dtype=self.residual_out.weight.dtype)).float()
        correction = self._condition_output_correction(base, correction)
        output = base.float() + correction
        if self.residual_out.weight.dtype != torch.float32:
        ...
        if tuple(output.shape) != expected_shape:
            raise RuntimeError(
                f"MEDCRAUp output-shape mismatch: got {tuple(output.shape)}, expected {expected_shape}."
            )
        return output
...
class URRDCRAUp(MEDCRAUp):
    ...
    _VALID_RELEASE_MODES = frozenset(("adaptive", "strict", "none"))
    ...
    def __init__(
        ...
        preserve_moments=True,
        center_correction=True,
        release_mode="adaptive",
        strict_ratio=0.10,
        release_power=1.0,
        energy_weighted_release=True,
        detach_moment_scale=True,
        detach_release=True,
        moment_scale_max=4.0,
        eps=1e-6,
        ...
        super().__init__(
            ...
            detach_moment_scale=detach_moment_scale,
            detach_energy_scale=True,
            moment_scale_max=moment_scale_max,
            eps=eps,
        )
        self.release_mode = str(release_mode).lower()
        self.strict_ratio = float(strict_ratio)
        self.release_power = float(release_power)
        self.energy_weighted_release = bool(energy_weighted_release)
        self.detach_release = bool(detach_release)

        if self.release_mode not in self._VALID_RELEASE_MODES:
            raise ValueError(
                "release_mode must be one of "
                f"{sorted(self._VALID_RELEASE_MODES)}, got {release_mode!r}."
            )
        if not 0.0 < self.strict_ratio <= 1.0:
            raise ValueError(
                "strict_ratio must satisfy 0 < ratio <= 1, "
                f"got {self.strict_ratio}."
            )
        if self.release_power <= 0.0:
            raise ValueError(
                "release_power must be positive, "
                f"got {self.release_power}."
            )

    @staticmethod
    def _validate_confidence(confidence, base):
        """Validate the DCRA confidence map against its corrected base feature."""
        if confidence.ndim != 4:
            raise ValueError(
                "confidence must be a 4D NCHW tensor, "
                f"got shape {tuple(confidence.shape)}."
            )
        if confidence.shape[0] != base.shape[0]:
            raise ValueError(
                "confidence/base batch mismatch: "
                f"{confidence.shape[0]} vs {base.shape[0]}."
            )
        if confidence.shape[1] != 1:
            raise ValueError(
                "confidence must have exactly one channel, "
                f"got {confidence.shape[1]}."
            )
        if confidence.shape[-2:] != base.shape[-2:]:
            raise ValueError(
                "confidence/base spatial mismatch: "
                f"{tuple(confidence.shape[-2:])} vs {tuple(base.shape[-2:])}."
            )
        if confidence.device != base.device:
            raise ValueError(
                "confidence/base device mismatch: "
                f"{confidence.device} vs {base.device}."
            )

    def _compute_channel_reliability(self, confidence, correction):
        """Return detached FP32 reliability of shape ``[B, C, 1, 1]`` in ``[0, 1]``."""
        ...
        mean_confidence = confidence_fp32.mean(dim=(2, 3), keepdim=True).expand(batch, channels, 1, 1)

        if self.energy_weighted_release:
            correction_energy = correction.float().square()
            energy_sum = correction_energy.sum(dim=(2, 3), keepdim=True)
            weighted_confidence = (correction_energy * confidence_fp32).sum(
                dim=(2, 3), keepdim=True
            ) / energy_sum.clamp_min(self.eps)
            reliability = torch.where(energy_sum > self.eps, weighted_confidence, mean_confidence)
        else:
            reliability = mean_confidence

        reliability = reliability.clamp(0.0, 1.0).pow(self.release_power)
        return reliability.detach() if self.detach_release else reliability

    def _route_output_correction(self, base, correction, confidence):
        """Center and route correction between the strict (M2) and released (M5) endpoints."""
        if base.shape != correction.shape:
            raise ValueError(
                "base and correction must have identical shapes, got "
                f"base={tuple(base.shape)}, correction={tuple(correction.shape)}."
        ...
        _, _, base_rms = self._spatial_mean_and_centered_rms(base, self.eps)
        correction_rms = correction_fp32.square().mean(dim=(2, 3), keepdim=True).add(self.eps).sqrt()
        strict_scale = torch.minimum(
            torch.ones_like(correction_rms),
            (self.strict_ratio * base_rms) / correction_rms.clamp_min(self.eps),
        )
        if self.detach_release:
            strict_scale = strict_scale.detach()

        if self.release_mode == "strict":
            effective_scale = strict_scale
        else:
            reliability = self._compute_channel_reliability(confidence, correction_fp32)
            effective_scale = strict_scale + reliability * (1.0 - strict_scale)
        effective_scale = effective_scale.clamp(0.0, 1.0)
        if self.detach_release:
            effective_scale = effective_scale.detach()
        return correction_fp32 * effective_scale

    def forward(self, x):
        if not isinstance(x, (list, tuple)) or len(x) != 2:
            raise TypeError("URRDCRAUp expects input as [deep_feature, lateral_feature].")
        ...
        if self.residual_out.weight.dtype == torch.float32:
            ...
            correction = self.residual_out(residual.to(dtype=self.residual_out.weight.dtype)).float()
        correction = self._route_output_correction(base, correction, confidence)
        output = base.float() + correction
        if self.residual_out.weight.dtype != torch.float32:
            output = output.to(dtype=deep.dtype)
        ...
        if tuple(output.shape) != expected_shape:
            raise RuntimeError(
                "URRDCRAUp output-shape mismatch: "
...
class DGMRLCERDCRAUp(LSMRLCERDCRAUp):
    ...
    def _validate_relaxation_map(self, relaxation, reference, name, spatial):
        ...
        # A support gate only controls how far the residual may depart from the
        # matched-moment endpoint.  If FP32 support statistics overflow during
        # a late validation pass, treating that location as unsupported (zero
        # relaxation) is conservative and keeps the residual path finite.
        relaxation = torch.nan_to_num(relaxation.float(), nan=0.0, posinf=0.0, neginf=0.0)
        return relaxation.clamp(0.0, self.moment_relax_max)
...
class BGDRP3Fuse(nn.Module):
    ...
    def __init__(self, c_p2, c_p3, config=None):
        ...
        nn.init.zeros_(self.detail_out.weight)
        # Trainer.optimizer_step() applies global norm clipping. A non-finite
        # gradient in any BGDR residual-branch parameter would otherwise make
        # that global rescaling contaminate otherwise finite gradients.
        for parameter in self.parameters():
            if parameter.requires_grad:
                parameter.register_hook(self._sanitize_bgdr_gradient)
    ...
    def _sanitize_bgdr_gradient(self, gradient):
        """Keep every BGDR residual-branch gradient finite before global clipping."""
