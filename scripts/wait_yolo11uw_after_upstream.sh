...
# The Python waiter logs terminal status/reason, then execs run_yolo11uw.sh.
exec "$PYTHON" -u "$PROJECT/tools/uw_chain.py" wait --variant "$VARIANT" --run-id "$RUN_ID" --upstream "$UPSTREAM"
