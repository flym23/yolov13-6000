...
# The Python waiter logs terminal status/reason, then execs run_yolo11uw_v3.sh.
exec "$PYTHON" -u "$PROJECT/tools/uwv3/chain.py" wait --variant "$VARIANT" --run-id "$RUN_ID" --upstream "$UPSTREAM"
