...
DATA=$ROOT/data.yaml
DURC_STATE=/home/room305/ZZF/yolov13yuan-6000/runs/durc_ablation/state.json
...
RESUME_LOG_TAG=${MEDCRA_RESUME_LOG_TAG:-$(date -u +%Y%m%dT%H%M%SZ)}
...
write_state() {
  MEDCRA_STATUS=$1 MEDCRA_STAGE=$2 MEDCRA_CODE=${3:-0} MEDCRA_RUN_ID=$RUN_ID MEDCRA_PID=$$ MEDCRA_STATE=$STATE MEDCRA_START_STAGE=$START_STAGE MEDCRA_RESUME_STAGE=$RESUME_STAGE \
    "$PY" -c 'import json,os; from pathlib import Path; from datetime import datetime,timezone; p=Path(os.environ["MEDCRA_STATE"])/"state.json"; d={"run_id":os.environ["MEDCRA_RUN_ID"],"status":os.environ["MEDCRA_STATUS"],"stage":os.environ["MEDCRA_STAGE"],"exit_code":int(os.environ["MEDCRA_CODE"]),"launcher_pid":int(os.environ["MEDCRA_PID"]),"dataset":"/home/room305/ZZF/URPC2020/data.yaml","epochs":300,"patience":40,"amp":False,"start_stage":os.environ["MEDCRA_START_STAGE"],"resume_stage":os.environ["MEDCRA_RESUME_STAGE"] or None,"dependency_state":"/home/room305/ZZF/yolov13yuan-6000/runs/durc_ablation/state.json","stage_order":["m0_original","m1_a4","m2_full","m3_no_moment","m4_no_center","m5_no_bound","m6_rho005","m7_rho020"],"updated_at":datetime.now(timezone.utc).isoformat()}; t=p.with_suffix(".tmp"); t.write_text(json.dumps(d,indent=2),encoding="utf-8"); t.replace(p)'
}
...
((START_INDEX >= 0)) || { echo "Unknown MEDCRA_START_STAGE: $START_STAGE" >&2; exit 64; }
[[ -z "$RESUME_STAGE" || "$RESUME_STAGE" == "$START_STAGE" ]] || {
  echo "MEDCRA_RESUME_STAGE must equal MEDCRA_START_STAGE" >&2
  exit 64
}
...
if ((START_INDEX == 0)); then
  ...
  current=resume_preflight
  write_state running "$current"
  for ((index=0; index<START_INDEX; index++)); do
    prior_stage=${STAGES[$index]}
    for seed in 0 1 2; do
      name=medcra_${RUN_ID}_${prior_stage}_seed${seed}
...
for stage_index in "${!STAGES[@]}"; do
  ((stage_index >= START_INDEX)) || continue
  ...
  for seed in 0 1 2; do
    ...
    resume_args=()
    [[ "$stage" != "$RESUME_STAGE" ]] || resume_args=(--resume)
    train_log=$TRAIN/${name}.log
    [[ "$stage" != "$RESUME_STAGE" ]] || train_log=$TRAIN/${name}.resume_${RESUME_LOG_TAG}.log
    CUDA_VISIBLE_DEVICES=0 WANDB_DISABLED=true "$PY" "$ROOT/tools/train_medcra_worker.py" --root "$ROOT" --stage "$stage" --data "$DATA" --name "$name" --seed "$seed" --epochs 300 --patience 40 "${resume_args[@]}" > "$train_log" 2>&1 &
    pids+=("$!")
