...
while true; do
  if [[ -f "$SOURCE_STATE" ]]; then
    read -r source_status source_run_id < <("$PY" -c 'import json,sys; d=json.load(open(sys.argv[1])); print(d.get("status",""), d.get("run_id",""))' "$SOURCE_STATE")
    if [[ "$source_status" == complete ]]; then
      break
    fi
    if [[ "$source_status" == failed ]]; then
      echo "Source baseline/A4 chain failed; ME-DCRA will not start." >&2
      exit 70
    fi
  fi
  sleep 60
...
for stage in baseline_original a4_tau020; do
  for seed in 0 1 2; do
    test -s "$TEST/urpc2020_${source_run_id}_${stage}_seed${seed}/summary_metrics.json"
    test -s "$TEST/urpc2020_${source_run_id}_${stage}_seed${seed}/scale_ap_metrics.json"
  done
done
...
backup="$ROOT/.codex_backups/medcra_activate_$(date -u +%Y%m%dT%H%M%SZ)"
...
"$PY" "$ROOT/tools/collect_medcra_ablation.py" --root "$ROOT" --run-id "$RUN_ID" --source-run-id "$source_run_id" --stages m0_original m1_a4
...
test_one() {
  local name=$1
  CUDA_VISIBLE_DEVICES=0 WANDB_DISABLED=true "$PY" "$ROOT/test.py" --weights "$TRAIN/$name/weights/best.pt" --data "$DATA" --name "$name" --device 0 --batch 16 --imgsz 640 > "$TEST/${name}.log" 2>&1
...
for stage in "${STAGES[@]}"; do
  ...
  for seed in 0 1 2; do
    ...
    CUDA_VISIBLE_DEVICES=0 WANDB_DISABLED=true "$PY" "$ROOT/tools/train_medcra_worker.py" --root "$ROOT" --stage "$stage" --data "$DATA" --name "$name" --seed "$seed" --epochs 300 > "$TRAIN/${name}.log" 2>&1 &
    pids+=("$!")
  ...
  for pid in "${pids[@]}"; do wait "$pid" || failed=1; done
  ((failed == 0))
  completed=(m0_original m1_a4)
  for candidate in "${STAGES[@]}"; do
    completed+=("$candidate")
    [[ "$candidate" == "$stage" ]] && break
  done
  "$PY" "$ROOT/tools/collect_medcra_ablation.py" --root "$ROOT" --run-id "$RUN_ID" --source-run-id "$source_run_id" --stages "${completed[@]}"
done

current=complete
write_state complete "$current"
echo "ME-DCRA chain completed: run_id=$RUN_ID source_run_id=$source_run_id"
