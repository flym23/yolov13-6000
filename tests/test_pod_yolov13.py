...
def test_ocf_preserves_native_concat_promotion_under_autocast() -> None:
    module = OCFConcat(32, 48).eval()
    deep, lateral = torch.randn(1, 32, 16, 16), torch.randn(1, 48, 16, 16).bfloat16()
    with torch.no_grad(), torch.autocast("cpu", dtype=torch.bfloat16):
        output = module([deep, lateral])
    expected = torch.cat((deep, lateral), dim=1)
    assert output.dtype is expected.dtype
