#!/usr/bin/env python3
"""Aggregate S1--S3 SAMR-LCER-DCRA summaries, paired S1 comparisons, and reused baselines."""
...
from samr_lcer_dcra_experiments import REFERENCE_BASELINES, STAGE_ORDER, STRUCTURES  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="append",
        default=None,
        help="Existing 3-seed stage summary to retain as an unpaired historical reference.",
    )
...
def paired_statistics(control: list[dict], candidate: list[dict]) -> dict[str, dict]:
    ...
    if set(control_by_seed) != {0, 1, 2} or set(candidate_by_seed) != {0, 1, 2}:
        raise ValueError("paired SAMR analysis requires exactly seeds 0, 1, and 2 for S1 and each candidate")
    result = {}
...
def load_reference(path: Path) -> dict:
    """Read an existing collector stage summary without treating a different batch as paired data."""
    if not path.is_file():
        raise FileNotFoundError(path)
    payload = json.loads(path.read_text(encoding="utf-8"))
    runs = payload.get("runs")
    if not isinstance(runs, list) or len(runs) != 3:
        raise ValueError(f"{path}: reference baseline must contain exactly three runs")
    if {int(row["seed"]) for row in runs} != {0, 1, 2}:
        raise ValueError(f"{path}: reference baseline must contain seeds 0, 1, and 2")
    metrics = payload.get("metrics_percent", {})
    if set(ALL_METRICS) - set(metrics):
        raise KeyError(f"{path}: missing reference metrics {sorted(set(ALL_METRICS) - set(metrics))}")
    return {
        "source": str(path),
...
def main() -> None:
    ...
    summary = {
        ...
        "comparison_rule": "Only S2/S3 versus same-batch S1 are paired; reused historical nearest baselines are unpaired references.",
    ...
    for stage in args.stages:
        ...
        if stage != "s1_matched_endpoint" and "s1_matched_endpoint" in args.stages:
            stage_summary["paired_vs_s1"] = paired_statistics(
                [row for row in records if row["stage"] == "s1_matched_endpoint"], stage_rows
            )
