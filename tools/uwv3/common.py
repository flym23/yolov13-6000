...
DATA = ROOT / 'data_urpc2020half.yaml'
...
def model_yaml(variant):
    ...
    return ROOT / 'ultralytics/cfg/models/11' / f'yolo11n_uw_v3_{variant}.yaml'
...
def run_root(variant, run_id):
    ...
    return ROOT / 'runs' / f'yolo11uw_v3_{variant}_{run_id}'
...
def required_files(variant):
    return [DATA, WEIGHTS, model_yaml(variant), Path(__file__).with_name('worker.py'),
            ...
            ROOT / 'ultralytics/utils/uw_v3_training.py',
            ...
            ROOT / 'scripts/run_yolo11uw_v3.sh', ROOT / 'scripts/wait_yolo11uw_v3_after_upstream.sh']
